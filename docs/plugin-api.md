...
- `policyProfiles` requires no special permission by default because policy presets can only tighten core validation.
...
Declares static templates or template ids. Template content must be sanitized before injection into the editor and must not contain real sensitive examples, PINs, keys, or remote resources.

### `policyProfiles`

Declares stricter publish-time policy presets. Policy plugins may raise minimum PIN length, require existing strong KDF profiles, or require metadata fields. They must not lower the default PIN/KDF security floor.
