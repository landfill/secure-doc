...
- 관리자 UI 한국어/영어 전환과 문서별 오프라인 viewer 언어 선택
...
관리자 UI는 한국어(`ko`)와 영어(`en`)를 지원합니다. 기본 언어는 한국어이며, 사용자가 선택한 관리자 표시 언어는 브라우저 storage가 아니라 Electron main process가 `userData` 아래의 로컬 설정 파일에 저장합니다.
