...
export const BUILT_IN_PLUGIN_MANIFESTS: PluginManifest[] = [
  ...
  },
  {
    ...
    version: "0.1.0",
    description:
      "중요 문서 발행용 보안 기준을 강화합니다. 활성화하면 PIN은 10-15자로 제한되고 낮은 PBKDF2 호환 모드는 사용할 수 없습니다.",
    category: "policy",
    permissions: [],
    contributes: {
      policyProfiles: [
        {
          ...
          label: "강화 PIN",
          description: "문서 발행 시 10-15자 PIN을 요구하고 600,000회 PBKDF2 호환 모드를 차단합니다."
        }
      ]
    }
  }
...
export function getEnabledPluginContributions(
  ...
  for (const manifest of manifests) {
    ...
    for (const profile of manifest.contributes.policyProfiles ?? []) {
